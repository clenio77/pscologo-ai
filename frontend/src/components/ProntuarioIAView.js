...
const ProntuarioIAView = ({ usuario, API_BASE_URL, pacienteSelecionado }) => {
  ...
  const formatarProntuarioIA = (texto) => {
    ...
    return (
      <div
        className="clinical-report-markdown"
        style={{ lineHeight: '1.7', color: '#e2e8f0', maxHeight: '550px', overflowY: 'auto', paddingRight: '10px' }}
      >
        {linhas.map((linha, idx) => {
          ...
          if (l.startsWith('-') || l.startsWith('*')) {
            return (
              <li key={idx} style={{ marginLeft: '1.5rem', marginBottom: '0.4rem', listStyleType: 'disc' }}>
                {l.replace(/^[-*]\s*/, '')}
              </li>
            );
          }
          ...
          if (partes.length > 1) {
            return (
              <p key={idx} style={{ marginBottom: '0.75rem' }}>
                {partes.map((p, i) =>
                  i % 2 === 1 ? (
                    <strong key={i} style={{ color: 'white' }}>
                      {p}
                    </strong>
                  ) : (
                    p
  ...
  return (
    <div
      style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '2rem', alignItems: 'stretch' }}
      className="prontuario-grid"
    >
      ...
      >
        ...
        <form onSubmit={handleAdicionarNota} style={{ marginBottom: '2rem' }}>
          <div className="form-group" style={{ marginBottom: '1rem' }}>
            <label
              style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.5rem', display: 'block' }}
            >
              Escrever nova anotação clínica da sessão presencial:
            </label>
            <textarea
        ...
        >
          ...
          <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>({listaNotas.length})</span>
        </h4>
          ...
          {carregandoNotas ? (
            ...
            >
              <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', margin: 0 }}>
                Nenhum relato presencial anotado para este paciente.
              </p>
            </div>
          ) : (
            listaNotas.map((nota) => (
      ...
      >
          ...
          <div>
            ...
            {prontuarioData && (
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                Última consolidação: {prontuarioData}
              </span>
            )}
          </div>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <h4 style={{ margin: 0, color: 'white', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Sparkles size={16} color="#8b5cf6" /> Resumo Pré-Sessão IA (Baseado no Diário)
            </h4>
            <button
              onClick={gerarResumoPreSessao}
        ...
        <div
          style={{
            ...
            display: 'flex',
            flexDirection: 'column',
            justifyContent: gerandoProntuario || !prontuarioConteudo ? 'center' : 'flex-start',
          }}
        >
          {gerandoProntuario ? (
            <div style={{ textAlign: 'center', padding: '3rem' }}>
              ...
              />
              <span className="loading-dots" style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>
                Consolidando anotações do terapeuta e desabafos do paciente com Gemini
              </span>
            </div>
          ) : prontuarioConteudo ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
              ...
              >
                Este prontuário inteligente funde as <strong>anotações clínicas presenciais</strong> que você salva à
                esquerda com os <strong>diários de áudio gravados</strong> pelo próprio paciente no aplicativo, traçando
                padrões evolutivos com o Gemini.
              </p>
              <button
